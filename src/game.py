...
def find_all_pos_in_word(word, char_to_find):
    return [pos for pos, char in enumerate(word) if char == char_to_find]


def change_chars_in_positions(word, list_of_positions, letter):
...
def step_of_game(answer_word, guessed_word):
    ...
    if input_char in guessed_word:
        all_inputs_of_word = find_all_pos_in_word(guessed_word, input_char)
        changed_word = change_chars_in_positions(
            answer_word, all_inputs_of_word, input_char)
        return changed_word
    else:
        return answer_word
